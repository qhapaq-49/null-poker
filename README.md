...
- AIは `FrequencyPolicy` で、MDF、pot odds、value/bluff比、ポジション、残り人数、ドロー、ブロッカー、簡易レンジ圧、メイドハンド品質/equity realization寄りのpot-control補正、局面別の目標bet/raise頻度を混ぜる。相手傾向readsはhand間で減衰しながら持ち越す。2人卓そのものは自己対局で残ったlow-donk系の `duelHeadsUp` を使い、多人数卓で残り2人になった局面は従来のtight-pressure寄り `headsUp` を使う。
