...
- AIは `FrequencyPolicy` で、MDF、pot odds、value/bluff比、ポジション、残り人数、ドロー、ブロッカー、簡易レンジ圧、メイドハンド品質/equity realization寄りのpot-control補正、HUでのドロー品質semi-bluff補正、6max multiwayでの弱いc-bet抑制、局面別の目標bet/raise頻度を混ぜる。相手傾向readsはhand間で減衰しながら持ち越す。2人卓そのものは自己対局で残ったlow-donk系の `duelHeadsUp` を使い、多人数卓で残り2人になった局面は従来のtight-pressure寄り `headsUp` を使う。
...
- `AI推奨表示` オプションで、現在局面の推奨手、近似EV、混合頻度、MDF、equityを必要なときだけ表示。
- `AI観戦` と `演出速度` で、AI同士の自動進行を見たり、Instantで自分の手番まで高速に進めたりできる。Fast/Normal/Slowでは各アクションを席上の一時バブルで表示する。
