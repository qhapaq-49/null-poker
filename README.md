...
出力はseat別のbb/100、VPIP、PFR、action count、ms/decision、教師ラベルへのtop-1/KLです。短いhand数のbb/100は分散が大きいので、改善比較では同じseedと十分なhand数で比較します。
...
`teacher` は `fold`, `check`, `call`, `bet`, `raise` の確率分布を想定しています。手に入るpreflop表、solverから切り出したpostflopスポット、公開データセットをこの形式に寄せれば同じ評価器で比較できます。
