...
出力はseat別のbb/100、VPIP、PFR、action count、flop c-bet/donk頻度、ms/decision、教師ラベルへのtop-1/KLです。`--villain jammer` はseat 0のAIを、seat 1のpreflop最大raise/all-in固定相手と戦わせます。`--villain all-jammer` ならhero以外を全員固定相手にします。短いhand数のbb/100は分散が大きいので、改善比較では同じseedと十分なhand数で比較します。
