...
- Equityはブラウザで回る範囲のMonte Carloサンプリング。postflopの少人数局面では候補手ごとに浅いrolloutも少量混ぜ、ポジション価値が行動順からも出るようにする。人数が増えるほどサンプル数を落として応答時間を保つ。
...
./bench.js --hands 400 --samples 24 --players 6 --lineup current,no-rollout --seed 42
./bench.js --hands 200 --samples 18 --players 6 --json > bench-result.json
```

出力はseat別/AI policy別のbb/100、VPIP、PFR、action count、flop c-bet/donk頻度、ms/decision、教師ラベルへのtop-1/KLです。`--villain jammer` はseat 0のAIを、seat 1のpreflop最大raise/all-in固定相手と戦わせます。`--villain all-jammer` ならhero以外を全員固定相手にします。`--lineup current,no-rollout,tight,loose` のように複数policyを同卓へ周期配置できます。短いhand数のbb/100は分散が大きいので、改善比較では同じseedと十分なhand数で比較します。
